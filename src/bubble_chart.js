...
function bubbleChart() {
  ...
  var LayerCenters = {
    "First Layer": { x: width / 3, y: height / 2 },
    "Second Layer": { x: 2 * width / 3, y: height / 2 }
  };

  // X locations of the Layer titles.
  var LayersTitleX = {
    "First Layer": 160,
    "Second Layer": width -160,
  };
  ...
  var fillColor = d3.scaleOrdinal()
    .domain(['Agriculture and Food,Development','Economics','Education','Employment','Energy','Environment','Finance and Investment','Governance','Industry and Services','Nuclear Energy','Science and Technology','Social Issues/Migration/Health','Taxation','Trade','Transport','Urban - Rural - Regional Development'])
    .range(['#add8e6','#8bc0ac','#67a973','#3d923b','#2c8400','#749000','#ad9a00','#e4a200','#ff982e','#ff7c55','#ff5c72','#ff2c8d','#e80d75','#c9054e','#aa012a','#8b0000']);
   ...
   * x force.
   */
  function nodeLayerPos(d) {
    return LayerCenters[d.layer].x;
  }
  ...
  function splitBubbles() {
    ...
    simulation.force('x', d3.forceX().strength(forceStrength).x(nodeLayerPos));
   ...
   */
  function showLayerTitles() {
    // Another way to do this would be to create
    // the Layer texts once and then just hide them.
    var LayersData = d3.keys(LayersTitleX);
    var Layers = svg.selectAll('.Layer')
      .data(LayersData);

    Layers.enter().append('text')
      .attr('class', 'Layer')
      .attr('x', function (d) { return LayersTitleX[d]; })
      .attr('y', 40)
  ...
  function showDetail(d) {
    ...
    var content = '<span class="name">Title: </span><span class="value">' +
                  ...
                  addCommas(d.value) +
                  ' views</span><br/>' +
                  '<span class="name">Layer: </span><span class="value">' +
                  d.layer +
                  '</span>';
...
// Load the data.
d3.csv('data/Edu_data.csv', display);
